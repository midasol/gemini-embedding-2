...
A multimodal RAG (Retrieval-Augmented Generation) chat system based on Gemini models.
It converts text, PDF, image, audio, and video files into vector embeddings and provides semantic search + AI streaming responses.

---

## Prerequisites

The following must be installed/prepared before getting started.
...
PostgreSQL with the pgvector extension is required for vector search.
...
### 4. Google Cloud Account and API Key
...
3. Go to **Cloud Storage** > "Create a bucket"
4. Go to **IAM & Admin** > "Service Accounts" and create a service account
5. Grant the **Storage Object Admin** role to the service account
...
# GCS service account JSON path (uses ADC if not provided)
...
> The IVFFlat vector index may be skipped if the table has no data. You can re-run it after the first embedding.
...
Open [http://localhost:3000](http://localhost:3000) in your browser, and it will automatically redirect to the `/chat` page.
...
Use the CLI or Admin UI to batch-embed files from a folder.

#### Batch Embedding via CLI

```bash
# Batch embed files from the data folder
pnpm pipeline -- ./data
...
> For security, the Admin UI only allows access to the `./data` and `./uploads` directories.

### 2. RAG Chat

1. Go to [http://localhost:3000/chat](http://localhost:3000/chat)
2. Type a message and the AI will search embedded documents for relevant content and respond
3. If image files are included in search results, they will be displayed with thumbnails

### 3. Single File Embedding During Chat

1. Click the clip icon in the chat input area to select a file
2. Include "embedding" in your message
   - Example: `Please embed this file`
3. The file will be uploaded to GCS and vector embeddings will be created
...
Confirm that the server correctly recognizes environment variables.
...
### Test 8: Verify Full Flow in Browser

1. Go to [http://localhost:3000/chat](http://localhost:3000/chat)
2. Check the conversation list in the left sidebar
3. Type a message: `What is the Gemini model?`
4. If a streaming response appears, it is working
5. Upload a file using the clip icon and type `Please embed this file`
...
| `pnpm lint` | ESLint code inspection |
...
# Re-run DB setup (index creation)
pnpm db:setup
```
...
> For detailed architecture and code analysis, see [docs/GUIDE.ko.md](./docs/GUIDE.ko.md).
