# Gemini RAG System - Detailed Guide Document

> Codebase analysis and usage guide for the multimodal RAG (Retrieval-Augmented Generation) system based on Gemini models
...
4. [Environment Setup and Installation](#4-environment-setup-and-installation)
...
12. [Core Configuration Reference](#12-core-configuration-reference)
...
This project is a **multimodal RAG system** utilizing Google Gemini AI models. It converts files of various formats (text, PDF, image, audio, video) into vector embeddings, stores them in PostgreSQL pgvector, and performs semantic search + AI response generation for user queries.

### Key Features

- **Batch Embedding Pipeline**: Bulk embedding of files in a folder (CLI + Web UI)
- **RAG Chat**: AI conversation based on vector search (streaming responses)
- **Single File Embedding**: Instant embedding via file upload during chat
- **Multimodal Support**: Text/PDF uses chunking, image/audio/video uses AI summary generation
...
│   │   ├── chat/
│   │   │   └── page.tsx              # Chat UI (sidebar + conversation window + input)
│   │   ├── admin/
│   │   │   └── pipeline/
...
│   │       ├── conversations/route.ts # Conversation list CRUD
│   │       ├── pipeline/
...
|--------|------|--------|------|
| `GEMINI_API_KEY` | Y | - | Google Gemini API key |
| `DATABASE_URL` | Y | - | PostgreSQL connection string |
| `GCS_BUCKET_NAME` | Y | - | GCS bucket name |
| `GCS_PROJECT_ID` | Y | - | Google Cloud project ID |
| `GOOGLE_APPLICATION_CREDENTIALS` | N | undefined | Service account JSON path |
| `GEMINI_EMBEDDING_MODEL` | N | `gemini-embedding-2-preview` | Embedding model |
| `GEMINI_CHAT_MODEL` | N | `gemini-3.1-pro-preview` | Chat model |
...
3. Create vector index (IVFFlat), filename index, and message composite index
...
Manages environment variables centrally; required variables are validated at runtime with `requireEnv()`.
...
A singleton client that communicates with the Google Gemini API.

| Export | Signature | Description |
|--------|----------|------|
| `genai` | `GoogleGenAI` instance | API client (singleton) |
| `generateEmbedding` | `(contents, taskType?) => Promise<number[]>` | Generate 3072-dimensional vector |
...
**Content Summary**: Generates detailed descriptions of images/videos using a Korean-language prompt.

### 5.3 db.ts - Database Connection

A PostgreSQL connection singleton based on Drizzle ORM + postgres-js.
...
Defines 3 tables and indexes using Drizzle ORM. (Details: see [8. Database Schema](#8-database-schema))
...
|-----------|-----------|------|
| Text/PDF | Text extraction -> chunking (2000 chars, 200 overlap) -> embedding per chunk | N records |
| Image/Audio/Video | Base64 -> multimodal embedding + AI summary | 1 record |

**Dependent Modules**: `gemini.ts`, `gcs.ts`, `file-parser.ts`, `db.ts`, `schema.ts`
...
| `buildRAGPrompt` | `(query, results) => string` | Generate prompt based on search results |
...
| `getFileCategory(fileName)` | File classification based on extension |
| `getMimeType(fileName)` | Extension to MIME type mapping |
...
| Export | Description |
|--------|------|
| `uploadToGCS(buffer, name, mime)` | GCS upload -> returns `/api/files/{path}` proxy URL |
| `downloadFromGCS(gcsPath)` | GCS download (path validation + path traversal prevention) |

**Security**:
- Randomize filenames with UUID
- Restrict access scope with `uploads/` prefix
- Block paths containing `..`

### 5.9 pipeline-state.ts - Pipeline State

An in-memory state store that manages pipeline progress.

| Export | Description |
|--------|------|
| `PipelineStatus` | Status interface (running, total, completed, succeeded, failed, logs) |
| `getStatus()` | Return shallow copy of current status |
| `resetStatus(total)` | Initialize at pipeline start |
| `updateStatus(partial)` | Partial status update |
| `addLog(log)` | Add log (maintains max 100, FIFO) |
...
Generates vector search -> RAG prompt -> streaming response for user queries.

**Request**:
```json
{
  "message": "Question to search (max 10,000 chars)",
...
5. Filter media attachments (top 95% similarity or above)
...
**Constraints**: Max 100MB, only supported extensions allowed
...
|--------|------|------|------|
| GET | List query (sorted by updatedAt, newest first) | - | `Conversation[]` |
...
**Security Validation**:
- `ALLOWED_BASE_DIRS`: Only `./data`, `./uploads` allowed
- Path traversal (`../`) blocked

**Behavior**: Returns `{ started: true }` immediately, then processes in background
...
Serves files stored in GCS via proxy.
...
| `/chat` | `ChatPage` | Chat interface (sidebar + conversation window + input) |
...
- **Conversation List**: `GET /api/conversations` (re-fetched on activeId change)
- **New Conversation**: `onNew()` callback
- **Delete Conversation**: `DELETE /api/conversations?id=` (delete button revealed on group-hover)
...
- **Message Rendering**: User (right-aligned) / Assistant (left, Markdown)
- **Markdown**: `react-markdown` + `remark-gfm` (tables, code, links support)
...
- **Selected File**: Badge display + delete button
...
- **Statistics**: Success (green) / Failed (red) / Pending count
- **Logs**: Recent processing results in ScrollArea (filename, status, duration)
...
|       +-- parseStreamChunk() -> attachments + text separation
|
...
### 8.1 Table Relationship Diagram
...
messages (N)
    |
    | attachments JSONB -> search result references
    v
...
- `idx_embeddings_file_name`: Filename search
...
| `file_name` | varchar(500) | nullable | Referenced filename |
...
File Type Filtering (getFileCategory)
  |
Process 5 at a time in parallel (Promise.allSettled)
  | (each file)
GCS Upload -> Obtain proxy URL
  |
+-- Text/PDF: Text extraction -> Chunking (2000/200) -> Embedding per chunk
+-- Multimodal: Base64 -> Embedding + AI summary
  |
Save to PostgreSQL embeddings table
  |
Status Update (pipeline-state.ts)
```

- **Concurrent Processing**: 5 files in parallel
- **Retry**: 3 retries on failure, then skip
- **Status Tracking**: In-memory, 1-second polling from Admin UI
...
pgvector Cosine Similarity Search (top 5)
  |
Media Filtering (image/video, similarity >= top value * 0.95)
  |
RAG Prompt Construction (search results + user question)
  |
Gemini 3.1 Pro Streaming Response
  |
Send Attachment Metadata + Text Streaming
  |
...
```
Select file in Chat UI + enter "embedding" keyword
  |
POST /api/embed (FormData)
  |
File Validation (size 100MB, supported types)
  |
embedFile(buffer, fileName) -> GCS + Embedding + DB Save
  |
...
2. Create embeddings, conversations, messages tables
3. Create vector index (IVFFlat), filename index, message index
...
3. Run `embedFile()` in parallel, 5 at a time
4. Output results
...
## 12. Core Configuration Reference
...
| Conversation Title Max Length | 200 characters (auto-generated 30 chars) | `schema.ts`, `chat/route.ts` |
...
| GCS Proxy | Proxy URL instead of public URL | `gcs.ts`, `files/route.ts` |
| Filename Randomization | UUID-based GCS filename generation | `gcs.ts` |
...
| HTTPS | Requires deployment environment configuration |
